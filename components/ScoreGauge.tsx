            ...
            <div className="flex items-end justify-between mb-4">
                <div className="flex flex-col">
                    ...
                    <div className="flex items-center gap-3">
                        <h3 className={`text-5xl font-extrabold ${config.textColor} tracking-tight tabular-nums transition-colors duration-500`}>
                            {displayScore}%
                        </h3>
                        {/* Emoji Indicator - Scales in when score stabilizes */}
                        <span 
                            className={`text-4xl filter drop-shadow-sm animate-float cursor-help transition-all duration-700 transform ${displayScore > 5 ? 'opacity-100 scale-100' : 'opacity-0 scale-50'}`} 
                            title={config.label}
                        >
                            {config.emoji}
                        </span>
                ...
                <div className={`px-4 py-1.5 rounded-full font-bold text-sm flex items-center gap-2 ${config.bgLight} ${config.textColor} border ${config.borderColor} mb-2 animate-enter-smooth`} style={{ animationDelay: '0.2s' }}>
                    {score >= 85 ? <Check size={16} /> : <Info size={16} />}
                    {config.label}
                </div>
            </div>

            {/* Visual Bar */}
            <div className="h-6 w-full bg-slate-100 rounded-full overflow-hidden shadow-inner border border-slate-200 relative">
                ...
                {/* The Bar */}
                <div 
                    className={`h-full ${config.color} rounded-full shadow-lg relative transition-all duration-[1500ms] cubic-bezier(0.16, 1, 0.3, 1) flex items-center justify-end pr-2`}
